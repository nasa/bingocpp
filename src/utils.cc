...
std::vector<Eigen::ArrayXXd> calculate_partials(Eigen::ArrayXXd x) {
    ...
    for (int i = 0; i < break_points.size() - 1; ++i) {
        ...
        time_deriv_no_edge = time_deriv.block(3, 0, time_deriv_no_edge.rows(), time_deriv.cols());
...
Eigen::ArrayXXd savitzky_golay(Eigen::ArrayXXd y, int window_size, int order, int deriv) {
